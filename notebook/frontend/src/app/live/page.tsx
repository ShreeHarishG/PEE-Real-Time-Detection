  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800">
      ...
      <div className="top-navbar shrink-0 bg-white border-b border-slate-200">
        ...
        >
          <Video className="h-3 w-3" /> Video Analysis
        </a>
        ...
        <div className="py-6">
          <div className="mx-auto max-w-[1400px] px-6 md:px-8">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-slate-800 tracking-tight">EdgeVision Live Dashboard</h1>
                <p className="text-sm text-slate-500 mt-1">Real-time AI monitoring of live camera feeds.</p>
              </div>
              ...
              <div className="flex gap-3">
                ...
                {uploadedVideo?.isCamera && (
                  <span className="text-xs font-medium px-2 py-1 rounded bg-indigo-50 text-indigo-600 border border-indigo-100 mt-1 flex items-center">
                    {uploadedVideo.name}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-3">
                {uploadedVideo && (!activeJob || ['stopped', 'completed', 'failed', 'camera_unavailable'].includes(activeJob.status)) && (
                  <button className="btn-primary text-sm flex items-center gap-2" onClick={startProcessing}>
                    <Play className="h-4 w-4" /> Start Live Monitoring
                  </button>
                )}
            ...
            {jobState === 'empty' && (
              <div className="glass-card p-16 text-center flex flex-col items-center justify-center animate-fade-in min-h-[500px] border-dashed border-2 border-gray-200 bg-white">
                <div className="w-24 h-24 rounded-full flex items-center justify-center mb-6" style={{ background: 'rgba(79, 70, 229, 0.06)' }}>
                  <Camera className="h-10 w-10" style={{ color: '#4f46e5' }} />
                </div>
                <h2 className="text-2xl font-bold text-gray-900 mb-3">Connect Live Camera</h2>
                <p className="text-gray-500 max-w-md mb-8 text-base">Select an IP camera or webcam from your configured sources to begin real-time monitoring.</p>
                <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md justify-center">
                  <button className="btn-primary flex-1 py-3 text-base flex justify-center items-center" onClick={() => setActiveModal('cameras')}>
                    <Camera className="h-5 w-5 mr-2" /> View Camera List
                  </button>
                </div>
              </div>
            )}
